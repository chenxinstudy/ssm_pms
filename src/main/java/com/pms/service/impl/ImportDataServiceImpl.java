...
public class ImportDataServiceImpl implements IImportDataService {
  ...
  public String insertMATable(MultipartFile excel) throws Exception {
    ...
    if (fileName.endsWith(".xls")) {
      String path = "D://PMSexcel/monthly_attendance_table/";
      ...
      for (MonthlyAttendance temp : monthlyAttendanceList) {
        ...
        mon =
            monthlyAttendanceMapper.selectByeIdAndDate(
                temp.getEmployee().geteId(), dateFormat.format(temp.getAttendanceTime()));
        if (mon == null) {
  ...
  public String insertReissueTable(MultipartFile excel) throws Exception {
    ...
    // 限制上传文件的类型
    if (fileName.endsWith(".xls")) {
      String path = "D://PMSexcel/monthly_reissue_table/";
      File fileDirectory = new File(path);
      if (!fileDirectory.exists()) {
        fileDirectory.mkdirs();
      }
      ...
      for (Salary salary : salaryList) {
        ...
        Salary temp =
            salaryMapper.selectByEidAndTimeAndStatus(
                salary.getEmployee().geteId(), dateFormat.format(salary.getsTime()), 0);
        // 如果数据库表salary不存在此记录，直接插入
        if (temp == null) {
          ...
          // 修改个人所得税
          salary.setIndividualIncomeTax(0.00);
          // 修改应发工资
          salary.setShouldPay(0.00);
          // 修改实发工资
          salary.setActualPay(0.00);

          salary.setsState(0);
          salaryMapper.insertSelective(salary);
        } else {
          // 如果数据库表salary已存在此记录，修改补发金额数据
          ...
          // 考勤扣费 4项
          double attendance =
              temp.getLatePay() + temp.getEarlyPay() + temp.getSickPay() + temp.getThingPay();

          double incomeTax = 0;
          // 纳税超额部分
          double taxable = shouldPay + insurances + attendance - temp.getBusinessTravelPay() - 3500;
